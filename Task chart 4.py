...
def run_desktop_app() -> int:
    ...
    class TaskChartDesktopApp(tk.Tk):
        ...
        def _configure_styles(self) -> None:
            ...
            style.configure("Horizontal.TProgressbar", background="#4f46e5", troughcolor="#e9edf5",
                            bordercolor="#ffffff", lightcolor="#4f46e5", darkcolor="#4f46e5")
        ...
        def _build_layout(self) -> None:
            ...
            self.ttk_mod.Button(actions, text="+ Add Task", style="Primary.TButton",
                                command=self._open_add_task_dialog).grid(row=0, column=0, padx=(0, 8))
            self.ttk_mod.Button(actions, text="Export Markdown", style="Secondary.TButton",
                                command=self._export_markdown).grid(row=0, column=1, padx=(0, 8))
            self.ttk_mod.Button(actions, text="Reset Status", style="Secondary.TButton",
                                command=self._reset_statuses).grid(row=0, column=2)
            ...
            self.summary_inner_frame = self.ttk_mod.Frame(summary_card, style="Card.TFrame")
            self.summary_inner_frame.grid(row=1, column=0, sticky="nsew", pady=(12, 0))

        def _sort_tasks(self) -> None:
            # High priority = 1, Medium = 2, Low = 3
            priority_map = {"High": 1, "Medium": 2, "Low": 3}
            for category in TASKS:
                # Sort by priority first, then alphabetically by task name
                TASKS[category].sort(key=lambda t: (priority_map.get(t.priority, 4), t.name))

        def _build_summary_ui(self) -> None:
            for widget in self.summary_inner_frame.winfo_children():
                widget.destroy()
            self.summary_widgets.clear()

            for category in TASKS.keys():
                lbl = self.ttk_mod.Label(self.summary_inner_frame, background="#ffffff", font=("Arial", 10, "bold"))
                lbl.pack(anchor="w", pady=(12, 2))
                pb = self.ttk_mod.Progressbar(self.summary_inner_frame, orient="horizontal", length=220,
                                              mode="determinate", style="Horizontal.TProgressbar")
                pb.pack(anchor="w", fill="x")
                self.summary_widgets[category] = (lbl, pb)

            self.ttk_mod.Separator(self.summary_inner_frame, orient="horizontal").pack(fill="x", pady=(20, 12))

            lbl = self.ttk_mod.Label(self.summary_inner_frame, background="#ffffff", foreground="#172033",
                                     font=("Arial", 11, "bold"))
            lbl.pack(anchor="w", pady=(0, 2))
            pb = self.ttk_mod.Progressbar(self.summary_inner_frame, orient="horizontal", length=220, mode="determinate",
                                          style="Horizontal.TProgressbar")
            pb.pack(anchor="w", fill="x")
            self.total_widgets = (lbl, pb)
        ...
        def _build_task_tabs(self) -> None:
            for category, tasks in TASKS.items():
                ...
                self.ttk_mod.Label(panel, text=f"{category} Tasks", style="CardTitle.TLabel").grid(row=0, column=0,
                                                                                                   sticky="w")
                ...
                task_frame.bind("<Configure>", lambda e, c=canvas: c.configure(scrollregion=c.bbox("all")))
                canvas.bind("<Configure>", lambda e, c=canvas, w=canvas_window: c.itemconfigure(w, width=e.width))
        ...
        def _build_rows(self, category: str, tasks: list[Task], parent) -> None:
            ...
            headers = ["Done", "Task", "Priority", "Status", "Edit", "Delete"]
            for col, text in enumerate(headers):
                self.ttk_mod.Label(parent, text=text, style="Header.TLabel", padding=(8, 6)).grid(row=0, column=col,
                                                                                                  sticky="ew")
            ...
            for row, task in enumerate(tasks, start=1):
                ...
                done_var.trace_add("write", lambda *_args, t=task, v=done_var, sl=status_label,
                                                   tl=task_label: self._change_status(t, v, sl, tl))
                ...
                self.ttk_mod.Button(parent, text="Edit", style="Action.TButton",
                                    command=lambda c=category, t=task: self._open_edit_dialog(c, t)).grid(row=row,
                                                                                                          column=4,
                                                                                                          sticky="e",
                                                                                                          pady=4,
                                                                                                          padx=(0, 8))
                self.ttk_mod.Button(parent, text="Delete", style="Danger.TButton",
                                    command=lambda c=category, t=task: self._delete_task(c, t)).grid(row=row, column=5,
                                                                                                     sticky="e", pady=4)
        ...
        def _change_status(self, task: Task, variable, status_label, task_label) -> None:
            ...
            task_label.configure(style="DoneTask.TLabel" if is_done else "Task.TLabel")
            self._update_summary()

        def _open_add_task_dialog(self) -> None:
            self._open_task_dialog(title="Add New Task")

        def _open_edit_dialog(self, category: str, task: Task) -> None:
            self._open_task_dialog(title="Edit Task", task=task, current_category=category)

        def _open_task_dialog(self, title: str, task: Task | None = None, current_category: str = "") -> None:
            ...
            if task: name_entry.insert(0, task.name)
            ...
            cat_cb.set(current_category if current_category else list(TASKS.keys())[0])
            ...
            pri_cb.set(task.priority if task else "Medium")
            ...
            def save_task():
                ...
                selected_cat = cat_cb.get()
                selected_pri = pri_cb.get()

                if task:  # Editing existing
                    task.name = name
                    task.priority = selected_pri
                    if selected_cat != current_category:
                        TASKS[current_category].remove(task)
                        TASKS[selected_cat].append(task)
                else:  # Adding new
                    TASKS[selected_cat].append(Task(name, selected_pri))

                self._refresh_ui()
                dialog.destroy()

            self.ttk_mod.Button(btn_frame, text="Save Task", style="Primary.TButton", command=save_task).pack(
                side="right", padx=(8, 0))
        ...
        def _refresh_ui(self) -> None:
            self._sort_tasks()  # Ensure tasks are sorted by High -> Medium -> Low before drawing
        ...
        def _update_summary(self) -> None:
            ...
            for category, rows in self.task_vars.items():
                ...
                if category in self.summary_widgets:
                    lbl, pb = self.summary_widgets[category]
                    lbl.configure(text=f"{category} ({done_count}/{task_count})")
                    pb["maximum"] = task_count if task_count > 0 else 1
                    pb["value"] = done_count

            if self.total_widgets:
                t_lbl, t_pb = self.total_widgets
                t_lbl.configure(text=f"Overall Progress: {total_done}/{total_tasks}")
                t_pb["maximum"] = total_tasks if total_tasks > 0 else 1
                t_pb["value"] = total_done
