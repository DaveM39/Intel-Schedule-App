...
class ScheduleApp:
    ...
    def _create_schedule_data(self):
        self.schedule = {
            # Data is the same, removed for brevity
        ...
        # Store original tags for easy reset
        self.original_tags = {}
        for day_key, info in self.schedule.items():
            for idx, activity in enumerate(info["activities"]):
                iid = f"day{day_key}_item{idx}"
                self.original_tags[iid] = (activity[2],)
    ...
    def _populate_notebook(self):
        ...
        for day_key, info in sorted(self.schedule.items(), key=lambda item: int(item[0])):
            ...
            for idx, (t, act, cat) in enumerate(info["activities"]):
                iid = f"day{day_key}_item{idx}"
                tree.insert("", END, values=(t, act), tags=self.original_tags.get(iid), iid=iid)
    ...
    def edit_activity_dialog(self, event, day_key, tree):
        ...
        def on_save():
            ...
            self.original_tags[item_iid] = (new_category,)  # Update original tag
    ...
    def load_schedule_data(self, file_path):
        try:
            ...
            self._create_schedule_data()  # Re-create original tags
            self._populate_notebook()
    ...
    def filter_by_category(self, category_to_filter):
        """NEW FILTER LOGIC: Re-tags all items based on the filter."""
        try:
        ...
        for iid in tree.get_children():
            original_tag = self.original_tags.get(iid, ('',))[0]
            if original_tag == category_to_filter:
                tree.item(iid, tags=(original_tag,))
            else:
                tree.item(iid, tags=(original_tag, "dimmed"))

    def clear_filter(self):
        """NEW CLEAR LOGIC: Clears filter on the current tab only."""
        try:
            selected_tab_widget = self.root.nametowidget(self.notebook.select())
            tree = selected_tab_widget.treeview
        except (KeyError, AttributeError):
        ...
        for iid in tree.get_children():
            original_tag = self.original_tags.get(iid)
            if original_tag:
                tree.item(iid, tags=original_tag)
    ...
    def show_yearly_calendar(self):
        ...
        self.calendar_window = ttk.Toplevel(self.root)  # Using ttk.Toplevel
        self.calendar_window.title("Yearly Cycle Calendar")
        self.calendar_window.geometry("900x700")

        # UI components are the same, removed for brevity
    ...
    def _update_calendar_display(self):
        """NEW CALENDAR RENDERING: Uses a colored Frame behind the Label."""
        ...
        for i in range(1, 13):
            ...
            for week_idx in range(TARGET_WEEKS_PER_MONTH_DISPLAY):
                for day_idx in range(7):
                    ...
                    # Create a colored frame for the day cell
                    ...
                    # Place a transparent label with the day number inside the frame
                    ...
                    day_label.pack(expand=YES, fill=BOTH)
