...
class ScheduleApp:
    ...
    def __init__(self, root: ttk.Window):
        ...
        # ── Calendar Specific Colors (Updated) ──────────────────────────────────
        self.cal_off_color = "#FFFFFF"  # White
        self.cal_on_early_color = "#FFF200"  # Yellow
        self.cal_on_late_color = "#FF9900"  # Orange
    ...
    def show_yearly_calendar(self):
        ...
        self.calendar_window.geometry("1180x950")

        self.calendar_year_var = tk.IntVar(value=datetime.date.today().year)
        nav_frame = ttk.Frame(self.calendar_window, padding=8)
        nav_frame.pack()
        ...
        # On Cycle (Yellow - Days 1&2)
        self.cal_legend_on_early_swatch = tk.Frame(cal_legend_frame, width=12, height=12, relief=SUNKEN, borderwidth=1)
        self.cal_legend_on_early_swatch.pack(side=LEFT, padx=(0, 2))
        ttk.Label(cal_legend_frame, text="Day").pack(side=LEFT, padx=(1, 10))

        # On Cycle (Orange - Days 3&4)
        self.cal_legend_on_late_swatch = tk.Frame(cal_legend_frame, width=12, height=12, relief=SUNKEN, borderwidth=1)
        self.cal_legend_on_late_swatch.pack(side=LEFT, padx=(0, 2))
        ttk.Label(cal_legend_frame, text="Night").pack(side=LEFT, padx=(0, 10))
        # ─────────────────────────────────────────────────────────
    ...
    def _update_calendar_display(self):
        """Uses a more robust grid layout for day numbers."""
        ...
        self.cal_legend_off_swatch.config(bg=self.cal_off_color)
        self.cal_legend_on_early_swatch.config(bg=self.cal_on_early_color)
        self.cal_legend_on_late_swatch.config(bg=self.cal_on_late_color)

        cols = 3
        TARGET_WEEKS_PER_MONTH_DISPLAY = 5

        for i in range(1, 13):
            month_frame = ttk.Labelframe(self.calendar_grid_frame, padding=12, bootstyle="secondary")
            month_frame.grid(row=(i - 1) // cols, column=(i - 1) % cols, padx=22, pady=22, sticky="nsew")
            ...
            for week_idx in range(TARGET_WEEKS_PER_MONTH_DISPLAY):
                for day_idx in range(7):
                    ...
                    if day_date_obj.month != i:
                        ...
                        # ── Modified Colour Logic ──────────────────────────────
                        if 0 <= cycle_pos <= 3:
                            # Days 1-4 of 8-day block (Off days)
                            day_bg = self.cal_off_color
                        elif 4 <= cycle_pos <= 5:
                            # Days 5-6 of 8-day block (1st half of On days)
                            day_bg = self.cal_on_early_color
                        else:  # 6 <= cycle_pos <= 7
                            # Days 7-8 of 8-day block (2nd half of On days)
                            day_bg = self.cal_on_late_color
                        # ───────────────────────────────────────────────────────
