...
class ScheduleApp:
    ...
    ▸ Interactive legend for filtering activities.
    """

    def __init__(self, root: ttk.Window):
        self.root = root
        self.root.title("Intel Technician Off-Days Schedule")
        self.root.geometry("1000x850")  # Increased height slightly for new layout
        ...
        self.calculate_dates()  # Calculate dates on startup
    ...
    def _build_widgets(self):
        ...
        # IMPROVEMENT: Increased height of the results window to 9
    ...
    def _build_legend(self, parent):
        ...
        for name, col in self.colors.items():
            # Use a lambda to capture the current category name for the callback
            callback = lambda n=name: self.filter_by_category(n)

            color_frame = tk.Frame(legend_frame, bg=col, width=16, height=16, relief=SUNKEN, borderwidth=1)
            color_frame.pack(side=LEFT)
            color_frame.bind("<Button-1>", lambda e, n=name: self.filter_by_category(n))

            label = ttk.Label(legend_frame, text=name.capitalize(), cursor="hand2")
    ...
    def _populate_notebook(self):
        ...
        for day_key, info in sorted(self.schedule.items(), key=lambda item: int(item[0])):
            ...
            # Configure colored tags for categories
            ...
            for idx, (t, act, cat) in enumerate(info["activities"]):
                # The item's primary tag is its category
            ...
            tab.treeview = tree  # Store reference to treeview in the tab frame
    ...
    def get_date_from_entry(self):
        """A safe way to get and parse the date from the entry widget."""
    ...
    def toggle_theme(self):
        ...
        self._apply_theme()
        # After theme change, re-apply any active filter to update dimmed colors
        self.clear_filter()

    def _apply_theme(self):
        style = self.root.style
        # Define the dimmed foreground color based on the theme
        dim_fg = "#999999" if not self.dark_mode else "#666666"
        style.configure("dimmed.TLabel", foreground=dim_fg)
    ...
    def filter_by_category(self, category_to_filter):
        """Highlights rows of a specific category and dims others in the current tab."""
        try:
            selected_tab_widget = self.root.nametowidget(self.notebook.select())
            tree = selected_tab_widget.treeview
        except (KeyError, AttributeError):
            return  # No treeview in the current tab or tab not found

        dim_color = "#EAEAEA" if not self.dark_mode else "#3A3A3A"

        # Configure a 'dimmed' tag with a washed-out background color
        tree.tag_configure("dimmed", background=dim_color, foreground=self._get_text_color_for_bg(dim_color))

        for item_id in tree.get_children():
            item_tags = tree.item(item_id, "tags")
            if item_tags and item_tags[0] == category_to_filter:
                # Remove the dimmed tag if it exists
                new_tags = [tag for tag in item_tags if tag != "dimmed"]
                tree.item(item_id, tags=new_tags)
            else:
                # Add the dimmed tag
                new_tags = list(item_tags) + ["dimmed"]
                tree.item(item_id, tags=new_tags)

    def clear_filter(self):
        """Removes all filtering from all treeviews."""
        for tab_id in self.notebook.tabs():
            try:
                tab_widget = self.root.nametowidget(tab_id)
                tree = tab_widget.treeview
                # Restore original category tag for all items
                for item_id in tree.get_children():
                    item_tags = tree.item(item_id, "tags")
                    original_category = [tag for tag in item_tags if tag in self.colors]
                    if original_category:
                        tree.item(item_id, tags=original_category)
            except (KeyError, AttributeError):
                continue
    ...
    def show_yearly_calendar(self):
        ...
        # FIX: Use ttk.Toplevel for proper theme inheritance
    ...
    def _update_calendar_display(self):
        ...
        # Use ttkbootstrap's way of getting theme colors
        style = self.root.style
        text_color_default = style.colors.get('fg')
