...
class ScheduleApp:
    ...
    ▸ Add new activities directly to the schedule. # NEW FEATURE
    ▸ Automatic highlighting of the current day's tab. # NEW FEATURE
    """
    ...
    def __init__(self, root: tk.Tk):
        ...
        self.start_date_obj = None  # MODIFIED FOR NEW FEATURE: Ensure it's initialized
        ...
        # MODIFIED FOR NEW FEATURE: Call calculate_dates and highlight tab on startup
    ...
    def _populate_notebook(self):  # MODIFIED FOR NEW FEATURE: Add Activity button
        ...
        for day_key, info in sorted(self.schedule.items(), key=lambda item: int(item[0])):
            ...
            header_frame = ttk.Frame(tab)  # MODIFIED FOR NEW FEATURE: Frame for title and button
            ...
            tree.column("activity", width=650)  # Adjusted width slightly
            ...
            tree_frame = ttk.Frame(tab)  # MODIFIED FOR NEW FEATURE: Frame for tree and scrollbar
            tree_frame.pack(fill=tk.BOTH, expand=True, padx=6, pady=(4, 10))

            vsb = ttk.Scrollbar(tree_frame, orient="vertical", command=tree.yview)  # Scrollbar inside tree_frame
            tree.configure(yscrollcommand=vsb.set)

            vsb.pack(side=tk.RIGHT, fill=tk.Y)
            tree.pack(side=tk.LEFT, fill=tk.BOTH, expand=True)

            tab.treeview = tree  # Store reference to treeview
    ...
    # NEW FEATURE: Dialog to add a new activity
    def add_activity_dialog(self, day_key):
        ...
        def on_save():
            ...
            new_activity_data = [new_time, new_activity, new_category]
            self.schedule[day_key]["activities"].append(new_activity_data)

            current_tab_widget = None
            # Find the tab widget by its text "Day X"
            for tab_id in self.notebook.tabs():
                if self.notebook.tab(tab_id, "text") == f"Day {day_key}":
                    current_tab_widget = self.notebook.nametowidget(tab_id)
                    break
            ...
            if current_tab_widget and hasattr(current_tab_widget, 'treeview'):
                ...
                new_item_iid = f"day{day_key}_item{new_item_idx}"  # Unique IID for the new item
                ...
                # Fallback, less efficient: self._populate_notebook()
    ...
    def edit_activity_dialog(self, event, day_key, tree):
        ...
        try:
            ...
            # Ensure the item_index is valid for the current schedule data
            if item_index >= len(self.schedule[day_key]["activities"]):
                messagebox.showerror("Error", "Item index out of sync. Please refresh or reload.", parent=self.root)
                return
    ...
    def load_schedule_data(self, file_path):
        try:
            ...
            # Ensure schedule structure is valid before assigning
            loaded_schedule = loaded_data.get("schedule")
            if isinstance(loaded_schedule, dict) and all(
                    isinstance(v, dict) and "activities" in v and "title" in v for v in loaded_schedule.values()):
                self.schedule = loaded_schedule
            else:
                messagebox.showwarning("Load Warning", "Schedule data in file is malformed. Loading default schedule.",
                                       parent=self.root)
                self._create_schedule_data()  # Load default if format is wrong

            self.user_name = loaded_data.get("userName", "")
            if hasattr(self, 'title_label'):  # Check if title_label exists
                base_text = '• 4‑On / 4‑Off Planner'  # Note the hyphen type
                if self.user_name:
            ...
            self._populate_notebook()  # Repopulate based on loaded schedule
            self.calculate_dates()  # This will also call _highlight_current_day_tab
            ...
            self.notes_text.insert("1.0", self.default_notes);
            self._populate_notebook()
            self.calculate_dates()  # Recalculate and highlight for default state
            ...
            messagebox.showerror("Load Error", f"Could not load schedule: {e}", parent=self.root)
            self._create_schedule_data();
            self.notes_text.delete("1.0", tk.END);
            self.notes_text.insert("1.0", self.default_notes);
            self._populate_notebook()
    ...
    def calculate_dates(self):  # MODIFIED FOR NEW FEATURE: calls highlight tab
        ...
        try:
            ...
            self.start_date_obj = None  # Set to None if date is invalid
            # Clear results if date is invalid
            self.results.config(state="normal")
            self.results.delete("1.0", tk.END)
            self.results.insert(tk.END, "Invalid date entered. Please correct.")
            self.results.config(state="disabled", fg="red")  # Optional: error color
            return
        lines = []
        for i in range(8):
        ...
        text_fg_color = self.style.lookup('TLabel', 'foreground')  # Get current theme's text color
        self.results.config(state="normal", fg=text_fg_color)
        self.results.delete("1.0", tk.END)
        self.results.insert(tk.END, "\n".join(lines))
        self.results.config(state="disabled")
        self._highlight_current_day_tab()  # NEW FEATURE: Highlight tab after calculation

    # NEW FEATURE: Method to highlight the current day's tab
    def _highlight_current_day_tab(self):
        if not hasattr(self, 'start_date_obj') or not self.start_date_obj:
            return

        if not self.notebook.tabs():  # Don't proceed if notebook isn't populated
            return

        today = datetime.date.today()
        try:
            delta_days = (today - self.start_date_obj).days
        except TypeError:  # If start_date_obj became None unexpectedly
            return

        if 0 <= delta_days <= 3:  # It's one of the 4 off-days displayed
            tab_index_to_select = delta_days
            try:
                if len(self.notebook.tabs()) > tab_index_to_select:
                    self.notebook.select(tab_index_to_select)
            except tk.TclError:
                # Could happen if tabs are not yet fully realized or some other edge case
                # print(f"Debug: TclError when trying to select tab index {tab_index_to_select}")
                pass
        # else: No specific tab to highlight if not within the 4 off-days
    ...
    def toggle_theme(self):
        ...
        # Recalculate dates to update results text color, which also calls _highlight_current_day_tab
        if hasattr(self, 'start_date_obj') and self.start_date_obj:
            self.calculate_dates()
        self._populate_notebook()  # Repopulate to apply new theme colors to treeview tags
    ...
    def _update_calendar_display(self):
        ...
        today_bg_cal = self.today_highlight_bg_dark if self.dark_mode else self.today_highlight_bg  # Renamed to avoid conflict
            ...
            for week_idx, week_data_list in enumerate(display_weeks_data):
                for day_idx, day_date_obj in enumerate(week_data_list):
                    ...
                    if not is_current_month_day:
                        ...
                        delta_days_cal = (day_date_obj - self.base_cycle_start_date_for_calendar).days  # Renamed
    ...
    def _apply_palette(self, *, bg: str, fg: str, textbg: str, treefield: str, treehead: str):
        ...
        self.style.configure("TButton", foreground=fg)  # Keep default button background for clam
        self.style.map("TButton", background=[('active', bg)], foreground=[('active', fg)])

        accent_button_bg = self._tint_color(treehead,
                                            1.1 if not self.dark_mode else 0.9)  # treehead or a distinct accent
        ...
        # Update main title label color
        if hasattr(self, 'title_label') and self.title_label:
            self.title_label.configure(bg=bg, fg=fg)

        # Update legend labels colors (ttk.Label are handled by style.configure("TLabel", ...))
        # for color_frame, label_widget in self.legend_item_frames.values():
        # label_widget.configure(background=bg, foreground=fg) # ttk.Labels are styled above
        ...
        self.style.map("Treeview", background=[("selected", "#6A9FB5")],  # Keep a distinct selection color
        ...
        if hasattr(self, 'calendar_window') and self.calendar_window.winfo_exists():
            ...
            # ttk widgets within calendar window will update via style, tk.Label need manual config if not themed
            self._update_calendar_display()  # This will re-apply colors to calendar days
    ...
    def set_user_name(self):
        ...
        if first is None: return  # User cancelled
        last = simpledialog.askstring("Last name", "Enter your last name:", parent=self.root)
        if last is None: return  # User cancelled

        self.user_name = f"{first.strip()} {last.strip()}"
        if not self.user_name.strip():  # If name is empty after stripping
            self.user_name = ""  # Treat as no name set
            if hasattr(self, 'title_label'):
                self.title_label.config(text=f"Intel Technician • 4‑On / 4‑Off Planner")
