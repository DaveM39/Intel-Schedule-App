...
class ScheduleApp:
    ...
    ▸ Edit activities in the schedule, including activity-specific notes. # MODIFIED FEATURE
    ▸ Graphical yearly calendar for on/off cycles.
    ▸ Add new activities directly to the schedule (with notes). # MODIFIED FEATURE
    ...
    def __init__(self, root: tk.Tk):
        ...
        self._build_widgets()
        self._create_schedule_data()  # This now includes empty notes
    ...
    def _create_schedule_data(self):  # MODIFIED: Added empty string for notes
        self.schedule = {
            "1": {"title": "Recovery after Night Shift", "activities": [
                ...
                ["8:00 PM", "Gym – Chest & Triceps", "gym", "Focus on incline press today."],
            ...
            "2": {"title": "Productive Focus", "activities": [
                ...
                ["10:45 AM", "Home maintenance / cleaning", "morning", "Fix leaky faucet in bathroom."],
                ["12:30 PM", "Lunch", "meal", ""],
                ["1:00 PM", "Take medicine (after lunch)", "medicine", ""],
                ["1:30 PM", "Deep coding session (2–3 h)", "coding", "Work on API integration module."],
                ["4:30 PM", "Learning – online course", "afternoon", "Chapter 5: Advanced Algorithms."],
                ["5:30 PM", "Rest / pre‑workout prep", "afternoon", ""],
                ["6:00 PM", "Pre‑workout meal", "meal", ""],
                ["7:00 PM", "Gym – Back & Biceps", "gym", "Try new pull-up variation."],
            ...
            "3": {"title": "Balance Day", "activities": [
                ...
                ["10:00 AM", "Protein breakfast", "meal", ""],
                ["10:30 AM", "Take medicine (after breakfast)", "medicine", ""],
                ["10:45 AM", "Meal prep for remaining days", "morning", "Cook chicken and rice."],
                ["12:30 PM", "Lunch", "meal", ""],
                ["1:00 PM", "Take medicine (after lunch)", "medicine", ""],
                ["1:30 PM", "Coding session (2–3 h)", "coding", "Refactor old utility functions."],
                ["4:30 PM", "Outdoor hobby / walk", "afternoon", "Park walk, 30 minutes."],
                ["5:30 PM", "Rest / pre‑workout prep", "afternoon", ""],
                ["6:00 PM", "Pre‑workout snack", "meal", ""],
                ["7:00 PM", "Gym – Shoulders & Abs", "gym", ""],
                ["9:00 PM", "Post‑workout dinner", "meal", ""],
                ["9:30 PM", "Take medicine (after dinner)", "medicine", ""],
                ["10:00 PM", "Reading / downtime", "evening", "Book: 'The Pragmatic Programmer'"],
                ["12:00 AM", "Bedtime", "evening", ""],
            ]},
            "4": {"title": "Social / Rest", "activities": [
                ...
                ["12:30 PM", "Lunch – meet a friend", "meal", "Meet Sarah at The Corner Cafe."],
                ["1:30 PM", "Free time / errands", "afternoon", "Post office, bank."],
                ["4:30 PM", "Prep for upcoming work block", "afternoon", "Charge devices, pack lunch."],
                ["6:00 PM", "Light gym – Stretch & Cardio", "gym", "20 min elliptical, full body stretch."],
    ...
    def _populate_notebook(self):
        ...
        for day_key, info in sorted(self.schedule.items(), key=lambda item: int(item[0])):
            ...
            remove_button = ttk.Button(header_frame, text="Remove Selected",
                                       command=lambda d=day_key, t=None: self.confirm_remove_activity(d, t))
            ...
            tree.column("activity", width=600)
            ...
            # Note: activity_item is [time, description, category, note]
            # Treeview only displays time and description
            for idx, activity_item in enumerate(info["activities"]):
                time_val, desc_val, cat_val = activity_item[0], activity_item[1], activity_item[2]
                tree.insert("", tk.END, values=(time_val, desc_val), tags=(cat_val,), iid=f"day{day_key}_item{idx}")
    ...
    def add_activity_dialog(self, day_key):  # MODIFIED: Added note field
        dialog = tk.Toplevel(self.root)
        dialog.title("Add New Activity")
        dialog.geometry("450x360")  # Adjusted height for note field
        ...
        ttk.Label(main_frame, text="Time:", font=self.normal_font).grid(row=0, column=0, padx=5, pady=8, sticky="w")
        ...
        ttk.Label(main_frame, text="Note:", font=self.normal_font).grid(row=3, column=0, padx=5, pady=8, sticky="nw")
        note_input_frame = ttk.Frame(main_frame)
        note_input_frame.grid(row=3, column=1, padx=5, pady=5, sticky="ew")
        note_text = tk.Text(note_input_frame, height=4, width=30, font=self.normal_font, wrap=tk.WORD, undo=True)
        note_scrollbar = ttk.Scrollbar(note_input_frame, orient=tk.VERTICAL, command=note_text.yview)
        ...
        def on_save():
            ...
            new_note = note_text.get("1.0", tk.END).strip()  # Get note
            ...
            new_activity_data = [new_time, new_activity, new_category, new_note]  # Include note
            ...
            if current_tab_widget and hasattr(current_tab_widget, 'treeview'):
                ...
                tree.insert("", tk.END, values=(new_time, new_activity), tags=(new_category,),
                            iid=new_item_iid)  # Tree still shows time/activity
    ...
    def edit_activity_dialog(self, event, day_key, tree):  # MODIFIED: Added note field
        item_iid = tree.focus()
        if not item_iid: return
        # item_values from tree are only time & activity_desc
        try:
        ...
        activity_data = self.schedule[day_key]["activities"][item_index]
        original_time = activity_data[0]
        original_activity_desc = activity_data[1]
        original_category = activity_data[2]
        original_note = activity_data[3] if len(activity_data) > 3 else ""  # Get note

        dialog = tk.Toplevel(self.root)
        dialog.title("Edit Activity")
        dialog.geometry("450x360")  # Adjusted height
        dialog.transient(self.root)
        dialog.grab_set()
        dialog.configure(bg=self.style.lookup('TFrame', 'background'))
        ...
        ttk.Label(main_frame, text="Activity:", font=self.normal_font).grid(row=1, column=0, padx=5, pady=8, sticky="w")
        activity_entry = ttk.Entry(main_frame, font=self.normal_font, width=35)
        activity_entry.insert(0, original_activity_desc)
        activity_entry.grid(row=1, column=1, padx=5, pady=8, sticky="ew")
        ...
        note_scrollbar = ttk.Scrollbar(note_input_frame, orient=tk.VERTICAL, command=note_text.yview)
        note_text.configure(yscrollcommand=note_scrollbar.set)
        note_text.insert("1.0", original_note)  # Populate with existing note
        note_scrollbar.pack(side=tk.RIGHT, fill=tk.Y)
        note_text.pack(side=tk.LEFT, fill=tk.BOTH, expand=True)

        def on_save():
            new_time = time_entry.get().strip()
            new_activity_desc = activity_entry.get().strip()
            new_category = category_var.get()
            new_note = note_text.get("1.0", tk.END).strip()  # Get note

            if not new_time or not new_activity_desc or not new_category:
                messagebox.showwarning("Input Error", "Time, Activity, and Category cannot be empty.", parent=dialog)
                return

            self.schedule[day_key]["activities"][item_index][0] = new_time
            self.schedule[day_key]["activities"][item_index][1] = new_activity_desc
            self.schedule[day_key]["activities"][item_index][2] = new_category
            self.schedule[day_key]["activities"][item_index][3] = new_note  # Save note

            tree.item(item_iid, values=(new_time, new_activity_desc),
                      tags=(new_category,))  # Tree still shows time/activity_desc
            text_color = self._get_text_color_for_bg(self.colors.get(new_category, "#FFFFFF"))
    ...
    def _refresh_single_treeview(self, day_key, tree):
        ...
        if day_key in self.schedule and "activities" in self.schedule[day_key]:
            for idx, activity_item in enumerate(self.schedule[day_key]["activities"]):
                time_val, desc_val, cat_val = activity_item[0], activity_item[1], activity_item[2]
                # Note is activity_item[3], but not displayed in tree
                new_iid = f"day{day_key}_item{idx}"
                tree.insert("", tk.END, values=(time_val, desc_val), tags=(cat_val,), iid=new_iid)
    ...
    def save_schedule_data(self, file_path):
        data_to_save = {
            "schedule": self.schedule,  # self.schedule now contains notes
    ...
    def load_schedule_data(self, file_path):  # MODIFIED: Handle migration for notes
        try:
            with open(file_path, "r") as f:
                loaded_data = json.load(f)

            raw_schedule_data = loaded_data.get("schedule")
            migrated_schedule = {}
            is_schedule_valid_or_migrated = True

            if isinstance(raw_schedule_data, dict):
                for day_k, day_v_info in raw_schedule_data.items():
                    if isinstance(day_v_info, dict) and \
                            "title" in day_v_info and \
                            "activities" in day_v_info and isinstance(day_v_info["activities"], list):

                        migrated_activities_list = []
                        for activity_entry in day_v_info["activities"]:
                            if isinstance(activity_entry, list) and len(
                                    activity_entry) >= 3:  # Must have at least time, desc, cat
                                # Ensure it has 4 elements (time, desc, cat, note)
                                current_activity_elements = list(activity_entry)  # Make a mutable copy
                                while len(current_activity_elements) < 4:
                                    current_activity_elements.append("")  # Add empty string for missing note
                                # Keep only the first 4 elements if more were somehow present
                                migrated_activities_list.append(current_activity_elements[:4])
                            else:
                                is_schedule_valid_or_migrated = False;
                                break  # Invalid activity format

                        if not is_schedule_valid_or_migrated: break
                        migrated_schedule[day_k] = {"title": day_v_info["title"],
                                                    "activities": migrated_activities_list}
                    else:
                        is_schedule_valid_or_migrated = False;
                        break  # Invalid day structure

                if is_schedule_valid_or_migrated:
                    self.schedule = migrated_schedule
                else:
                    messagebox.showwarning("Load Warning",
                                           "Schedule data in file is malformed or outdated. Loading default schedule.",
                                           parent=self.root)
                    self._create_schedule_data()
            else:
                messagebox.showwarning("Load Warning",
                                       "Schedule data structure in file is invalid. Loading default schedule.",
                                       parent=self.root)
                self._create_schedule_data()
    ...
    def calculate_dates(self):
        ...
        try:
            ...
            # Ensure tag exists before using it
            if "error_tag" not in self.results.tag_names():
    ...
    def toggle_theme(self):
        ...
        if hasattr(self, 'start_date_obj') and self.start_date_obj:
            ...
            if not self.start_date_obj and "Invalid date" in self.results.get("1.0", tk.END):
                if "error_tag" not in self.results.tag_names():  # Ensure tag exists
                    self.results.tag_configure("error_tag", foreground="red")
                # No need to re-insert, just ensure tag config and color are right.
            self.results.config(state="disabled")
