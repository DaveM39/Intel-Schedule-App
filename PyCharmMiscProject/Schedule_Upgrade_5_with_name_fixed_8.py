...
class ScheduleApp:
    ...
    def _build_widgets(self):
        ...
        self.title_label = ttk.Label(main_frame, text="Intel Technician • 4‑On / 4‑Off Planner", font=self.title_font, bootstyle="primary")
        ...
        ttk.Label(date_calc_frame, text="Current off-cycle start date (MM/DD/YYYY):").pack(side=LEFT, padx=(0, 8))
        self.date_entry = ttk.DateEntry(date_calc_frame, bootstyle="primary")
        self.date_entry.entry.insert(0, datetime.date.today().strftime("%m/%d/%Y"))
        self.date_entry.pack(side=LEFT)
        ttk.Button(date_calc_frame, text="Calculate Cycle", command=self.calculate_dates, bootstyle="primary-outline").pack(side=LEFT, padx=10)

        theme_button = ttk.Button(top_frame, text="Toggle Dark Mode", command=self.toggle_theme, bootstyle="info-outline")
        theme_button.pack(side=RIGHT)
        ttk.Button(top_frame, text="Set Name", command=self.set_user_name, bootstyle="secondary-outline").pack(side=RIGHT, padx=10)

        file_ops_frame = ttk.Frame(main_frame)
        file_ops_frame.pack(fill=X, pady=10)
        ttk.Button(file_ops_frame, text="Save Schedule", command=self.save_schedule_dialog, bootstyle="success-outline").pack(side=LEFT, padx=(0, 5))
        ttk.Button(file_ops_frame, text="Save Schedule As...", command=lambda: self.save_schedule_dialog(force_dialog=True), bootstyle="success-outline").pack(side=LEFT, padx=5)
        ttk.Button(file_ops_frame, text="Load Schedule", command=self.load_schedule_dialog, bootstyle="warning-outline").pack(side=LEFT, padx=5)
        ttk.Button(file_ops_frame, text="View Yearly Calendar", command=self.show_yearly_calendar, bootstyle="info-outline").pack(side=LEFT, padx=5)

        self.results = tk.Text(main_frame, height=3, font=self.small_font, relief=FLAT, state="disabled")
        ...
        ttk.Label(legend, text="Legend:", font=self.small_font).pack(side=LEFT, padx=(0, 6))
        for name, col in self.colors.items():
            color_frame = tk.Frame(legend, bg=col, width=16, height=16, relief=SUNKEN, borderwidth=1)
            color_frame.pack(side=LEFT)
            ttk.Label(legend, text=name.capitalize()).pack(side=LEFT, padx=(4, 8))

        notes_frame = ttk.Frame(main_frame)
        notes_frame.pack(fill=BOTH, pady=(10, 0), expand=YES)
        ttk.Label(notes_frame, text="Personal Notes:", font=self.header_font, bootstyle="primary").pack(anchor=W, pady=(0, 4))
    ...
    def _populate_notebook(self):
        ...
        for day_key, info in sorted(self.schedule.items(), key=lambda item: int(item[0])):
            ...
            for idx, (t, act, cat) in enumerate(info["activities"]):
                tree.insert("", END, values=(t, act), tags=(cat,), iid=f"day{day_key}_item{idx}")
    ...
    def edit_activity_dialog(self, event, day_key, tree):
        ...
        ttk.Button(button_frame, text="Cancel", command=dialog.destroy, bootstyle="danger-outline").pack(side=LEFT, padx=10)
    ...
    def save_schedule_data(self, file_path):
        data_to_save = {
            ...
            "startDate": self.date_entry.entry.get(),
    ...
    def load_schedule_data(self, file_path):
        try:
            ...
            self.date_entry.entry.insert(0, start_date)
            self._populate_notebook()
    ...
    def calculate_dates(self):
        try:
            self.start_date_obj = self.date_entry.get()
        except ValueError:
    ...
    def show_yearly_calendar(self):
        try:
            self.base_cycle_start_date_for_calendar = self.date_entry.get()
        except ValueError:
        ...
        nav_frame.pack()
        ttk.Button(nav_frame, text="< Prev", command=lambda: self._change_calendar_year(-1), bootstyle="primary").pack(side=LEFT, padx=5)
        ...
        ttk.Button(nav_frame, text="Next >", command=lambda: self._change_calendar_year(1), bootstyle="primary").pack(side=LEFT, padx=5)
        ttk.Button(nav_frame, text="Today", command=self._go_to_today_calendar, bootstyle="secondary").pack(side=LEFT, padx=20)
    ...
    def _update_calendar_display(self):
        ...
        text_color_default = self.root.style.colors.get('fg')
        ...
        for i in range(1, 13):
            ...
            ttk.Label(month_frame, text=calendar.month_name[i], font=self.calendar_month_font, anchor="center").grid(row=0, column=0, columnspan=7, pady=(5, 2))
            days_header = ["Su", "Mo", "Tu", "We", "Th", "Fr", "Sa"]
            for col_idx, day_name in enumerate(days_header):
                ttk.Label(month_frame, text=day_name, font=self.small_font, anchor="center").grid(row=1, column=col_idx, sticky="ew", padx=1, pady=1)
            ...
            for week_idx in range(TARGET_WEEKS_PER_MONTH_DISPLAY):
                for day_idx in range(7):
                    ...
                    day_bg = self.root.style.colors.get('bg')
                    ...
                    if day_date_obj.month != i:
                        ...
                        if 0 <= cycle_pos <= 3: day_bg = off_color
                        else: day_bg = on_color
